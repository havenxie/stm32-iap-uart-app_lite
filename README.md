...
4. 这套代码很简洁，对外围电路的要求只用到了一颗LED。用它来指示系统运行。
...
2. 工程配置：
    ...
    + 2.点击“Device”选择你的硬件平台（我这里是STM32F103RB）；
    + 3.点击“Target”找到下面的"IROM1",将Start中的0x8000000改成0x8003000（因为我们的app要从0x3000地址处开始存放）
    + 4.将Size中的值减去0x3000（直接16进制运算），然后将结果重新填入此处。（我这里是0x20000-0x3000=0x1D000,所以我填0x1D000,你要根据的片子进行计算一下）
